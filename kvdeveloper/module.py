...
def create_from_template(template_name: str, destination: str, variables: Dict[str, str]) -> None:
    """
...
def create_from_structure(template_name: str, structure_name: str, destination: str, variables: Dict[str, str]) -> None:
    ...
    if template_name=="blank":
        output=subprocess.run(["python","-m","kivymd.tools.patterns.create_project","MVC",".",f"{variables['project_name']}",f"python{version}","master","--use_hotreload","yes","--name_screen","SampleScreen"])
        ...
        with open(f"{destination}/View/SampleScreen/sample_screen.kv", "w", encoding="utf-8") as target_file:
            target_file.write(content)

            console.print(f"Updated file: [bright_white]{destination}/View/SampleScreen/sample_screen.kv[/bright_white]")
        
    elif template_name=="nav_toolbar":
        output=subprocess.run(["python","-m","kivymd.tools.patterns.create_project","MVC",".",f"{variables['project_name']}",f"python{version}","master","--use_hotreload","yes","--name_screen","HomeScreen","LoginScreen"])
        ...
        '''
        with open(f"{VIEW_BASE}/home_screen.py", "r", encoding="utf-8")  as template_file:
            content = template_file.read()
        
        with open(f"{destination}/View/HomeScreen/home_screen.py", "w", encoding="utf-8") as target_file:
            target_file.write(content)
            
            console.print(f"Updated file: [bright_white]{destination}/View/HomeScreen/home_screen.py[/bright_white]")
            
        with open(f"{template_path}/View/HomeScreen/home_screen.kv", "r", encoding="utf-8")  as template_file:
            content = template_file.read()
        
        with open(f"{destination}/View/HomeScreen/home_screen.kv", "w", encoding="utf-8") as target_file:
            target_file.write(content)
            
            console.print(f"Updated file: [bright_white]{destination}/View/HomeScreen/home_screen.kv[/bright_white]")
        ...
        with open(f"{destination}/View/base_screen.kv", "w", encoding="utf-8") as target_file:
            target_file.write(content)
            
            console.print(f"Updated file: [bright_white]{destination}/View/base_screen.kv[/bright_white]")
        
        '''
        updating login screen styles.
        '''
        with open(f"{template_path}/View/LoginScreen/login_screen.kv", "r", encoding="utf-8")  as template_file:
            content = template_file.read()
        
        with open(f"{destination}/View/LoginScreen/login_screen.kv", "w", encoding="utf-8") as target_file:
            target_file.write(content)
...
def project_info(project_name, template, structure, destination):
    ...
    info_panel=Panel(info_table,title=f"{project_name} - Info",title_align="left",border_style="bold green",padding=1)
