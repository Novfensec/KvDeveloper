...
from .module import create_from_template, create_from_structure, setup_build, project_info
...
@app.command()
def create(template: str = typer.Option(DEFAULT_TEMPLATE, help="Template for the project."), 
    ...
    typer.secho(f"Applying structure '{structure}': {structure_info}\n",fg=typer.colors.BRIGHT_CYAN)
...
def list_templates() -> None:
    ...
    """
    help_text=Text("\n Available Templates\n")
    templates_info=Text.assemble(("All the templates have inbuilt"),(" hot reload","bright_red"),(" system."))
    ...
    templates_box=Panel(template_table,title="Templates",title_align="left",expand=True)
    templates_info_box=Panel(templates_info,title="[bright_white]Templates Info[/bright_white]",expand=True,title_align="left",padding=1)
...
def list_structures() -> None:
    ...
    structures_box=Panel(structure_table,title="Structures",title_align="left",expand=True)
    typer.secho(help_text,fg=typer.colors.BRIGHT_WHITE)
